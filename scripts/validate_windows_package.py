...
def has_image_resource(name, files):
    """Accept literal images or the scale variants generated by msix/makepri."""
    path = name.replace('\\', '/').lower()
    if path in files:
        return True
    if 'resources.pri' not in files:
        return False
    image = PurePosixPath(path)
    return any(
        str(image.with_name(f'{image.stem}.scale-{scale}{image.suffix}')) in files
...
def validate(path, version):
    ...
    with zipfile.ZipFile(path) as package:
        ...
        for name in images:
            if not has_image_resource(name, files):
                errors.append(f'Missing package image resource: {name}')
